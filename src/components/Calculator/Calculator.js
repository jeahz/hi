...
const Calculator = () => {
  ...
  const computePrevious = {
    add: () => {
      if (operator === "add") {
        setCount(
          (count) => Math.round((Number(count) + Number(input)) * 10000) / 10000
        );
      }
    },
    subtract: () => {
  ...
  const handleClick = (e) => {
    if (e.target.id === "clear") {
      ...
      if (
        ...
        if (operator !== "add" && operator !== "") {
          ...
          if (output) {
            ...
            setCount(
              (count) =>
                Math.round((Number(count) + Number(input)) * 10000) / 10000
            );
          }
        }
      ...
      if (
        (!output && !isInputDecimal) ||
        (output && `${output}`.match(/\./g) === null)
      ) {
        if (output) {
          setInput(`${output}`);
          setDisplay(`${output}.`);
