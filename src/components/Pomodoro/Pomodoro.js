...
const Clock = () => {
  ...
  return (
    ...
    <audio src="https://res.cloudinary.com/dymnej9mt/video/upload/v1632633269/mixkit-censorship-beep-1082_gfemv9.wav" id="beep"></audio>
    <div className="lengths-container">
    <div className="length-container">
    <div className="length-label" id="break-label">Break Length</div>
    <div className="length-control">
    <button id="break-increment" onClick={handleClick}><i className="fas fa-chevron-up"></i></button>
    <div id="break-length">{breakLength}</div>
    <button id="break-decrement" onClick={handleClick}><i className="fas fa-chevron-down"></i></button>
    </div>
    </div>
    <div className="length-container">
    <span className="length-label" id="session-label">Session Length</span>
    <div className="length-control">
    <button id="session-increment" onClick={handleClick}><i className="fas fa-chevron-up"></i></button>
    <div id="session-length">{sessionLength}</div>
    <button id="session-decrement" onClick={handleClick}><i className="fas fa-chevron-down"></i></button>
    </div>
    </div>
    </div>
    <div className="timer">
    <div id="timer-label">{isBreak? 'Break' : 'Session'}</div>
    <span id="time-left">{minute < 10 ? '0' + minute : minute}:{second < 10 ? '0' + second : second}</span>
    <div id="timer-control">
    <button id="start_stop" onClick={handleClick}>
    <i className="fas fa-play"></i>
    <i className="fas fa-pause"></i></button>
    <button id="reset" onClick={handleClick}><i className="fas fa-step-backward"></i></button>
    </div>
    </div>
    </div>
    </>
