...
export function playDrum(type: string, vol01 = 0.7, when?: number): void {
  ...
  if (type === "kick") {
    // Body: sub-bass sweep. Higher floor (55Hz) so small speakers reproduce it.
    ...
    // Click transient: a short mid burst so the kick cuts through tiny speakers.
    const n = noiseSource(c, 0.03); const f = c.createBiquadFilter(); f.type = "bandpass"; f.frequency.value = 1800;
    const gc = g(0.03, 0.6); n.connect(f); f.connect(gc); n.start(t); n.stop(t + 0.03);
  } else if (type === "snare") {
    ...
    const n = noiseSource(c, dur); const f = c.createBiquadFilter(); f.type = "highpass"; f.frequency.value = 6000;
    const gg = g(dur, 0.9); n.connect(f); f.connect(gg); n.start(t); n.stop(t + dur);
  } else if (type === "clap") {
